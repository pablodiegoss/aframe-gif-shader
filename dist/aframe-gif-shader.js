		...
		/******/ 		var module = installedModules[moduleId] = {
			...
			/******/ ([
				...
				/***/ function(module, exports) {
					...
					var isGIF = function (gif){
						// GIF header contains this especificiation of type and version at the start of the file.
					...
					var createCanvas = function(frames, loadCnt, loopCnt, successCB, delayTimes){
					...
					var selectImageData = function(gif, pos){
						pos += 9;
					...
					var parseGIF = function(gif, delayTimes, loadCnt, loopCnt, frames){
						var pos = 0;
						var imageData = null;
						...
						while (gif[pos] && gif[pos] !== 0x3b) {
							...
							if (blockId === 0x21) {
								...
								// Using the current pos, walk on data until actual image ends.
								pos = selectImageData(gif, pos);
					...
					exports.parseGIF = function (gif, successCB, errorCB) {
						...
						var frames = [];
						if (isGIF(gif)) {
							console.log('gif:' + typeof gif)
							var imageInfo = parseGIF(gif, [], 0, 0, []);
							delayTimes = imageInfo.delayTimes;
							loadCnt = imageInfo.loadCnt;
							loopCnt = imageInfo.loopCnt;
							frames = loadGIF(gif)
							console.log(frames)	
							createCanvas(frames, loadCnt, loopCnt, successCB, delayTimes)
							// frames.forEach(function(frame){
							// 	createCanvas(frame)
							// })
							// frames.then(function(newFrames){
							// 		createCanvas(newFrames, loadCnt, loopCnt, successCB, delayTimes);
							// });
				...
				function loadGIF(nossoGif){
					// return new Promise(function(resolve,reject){
				
						gif = new GIF(nossoGif);
						var frames = gif.decompressFrames(true);
						
						var lsd = gif.raw.lsd
						
						tempCanvas.setAttribute("width",lsd.width)
						tempCanvas.setAttribute("height",lsd.height)
						
						var blobs = [];
						for(var i = 0; i < frames.length; i++){
							var imageData = gifCtx.createImageData(lsd.width, lsd.height)
							imageData.data.set(frames[i].patch);
							tempCtx.putImageData(imageData,0,0)
							tempCtx.drawImage(tempCanvas, lsd.width, lsd.height)
							var dataUrl = tempCanvas.toDataURL({type: "image/gif"});
							var image = new Image()
							image.src = dataUrl
							blobs.push(image.src)
